    ...
    ui(new Ui::fm_dados)
{
    ...
    setWindowTitle("Lista de compras");           //colocar título na janela
}
...
void fm_dados::on_btn_adicionar_clicked()
{
    ...
    if(ui->txt_produto->text().size() != 0 && ui->txt_preco->text().size() != 0 && ui->txt_quantidade->text().size() != 0){
        ...
        compra.setValor(ui->txt_preco ->text().toFloat());       //converter string para float
        compra.setQuantidade(ui->txt_quantidade->text().toInt());//converter string para int
       ...
       inserirCompraNaTabela(compra, qnt_row);
        ...
        ui->txt_produto->setFocus();       //colocar o cursor no txt_produto após inserir item na tabela
...
void fm_dados::salvar()
{
    ...
    if( GerenciadorDeArquivos::salvarConsumo(nomeArquivo,dadosTotais) ){
...
void fm_dados::carregar()
{
    ...
    if(GerenciadorDeArquivos::carregarConsumo(nomeArquivo,dadosTotais)){
...
void fm_dados::on_btn_fechar_clicked()
{
    QMessageBox::StandardButton resposta=QMessageBox::question(this,"Fechar","Deseja salvar?",QMessageBox::Yes|QMessageBox::No);
...
void fm_dados::on_tableWidget_cellDoubleClicked(int row, int column)
{
    QString editar;
    if(column == 1){
        editar = QInputDialog::getText(this, "Editar", "Qual é o produto?", QLineEdit::Normal);
    }else if(column == 2){
        editar = QInputDialog::getText(this, "Editar", "Qual é o valor?", QLineEdit::Normal);
    }else if(column == 3){
       editar= QInputDialog::getText(this, "Editar", "Qual é a quantidade?", QLineEdit::Normal);
    }
